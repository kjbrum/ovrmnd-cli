...
export function applyAuthentication(
  requestConfig: RequestInitWithParams,
  ...
  const authValueEnvVar =
    serviceConfig.authentication.tokenEnv ||
    serviceConfig.authentication.apiKeyEnv

  if (!authValueEnvVar) {
    throw new Error(
      'Authentication failed: Missing tokenEnv or apiKeyEnv in service configuration.',
    )
  }
  ...
  if (!authValue) {
    throw new Error(
      `Authentication failed: Environment variable ${authValueEnvVar} not found.`,
    )
  }
  ...
  if (authType === 'bearer') {
    ...
    if (!paramName) {
      throw new Error(
        'Authentication failed: Missing name for apiKey authentication.',
      )
    }
