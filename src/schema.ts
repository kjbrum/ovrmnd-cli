...
export function validateSchema(config: ServiceConfig): void {
  ...
  for (const endpoint of config.endpoints) {
    ...
    if (!endpoint.method || typeof endpoint.method !== 'string') {
      throw new Error(
        `Invalid endpoint "${endpoint.name}": missing method`,
      )
    }
    if (!endpoint.path || typeof endpoint.path !== 'string') {
      throw new Error(
        `Invalid endpoint "${endpoint.name}": missing path`,
      )
    }
  }
...
export function mapParameters(
  ...
  for (const param of pathParams) {
    ...
    if (args[paramName]) {
      ...
      const requiredParam = endpoint.parameters?.find(
        p => p.name === paramName && p.type === 'path' && p.required,
      )
      if (requiredParam) {
        throw new Error(
