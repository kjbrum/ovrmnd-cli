...
yargs(hideBin(process.argv))
    ...
    .command(
        ...
        async (argv) => {
            ...
            try {
                const [serviceName, endpointName] = (argv['service.endpoint'] as string).split('.');
                ...
                const { finalPath, body, query, headers } = mapParameters(endpointDefinition, argv);
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`API Error: ${response.status} ${response.statusText || ''}`, { cause: errorDetails });
                }
